...
def circle_mask(size=32, r=5, x_offset=0, y_offset=0):
...
def get_watermarking_mask(init_latents_w, args, device):
    ...
    print('hello navtegh yaha dekho')
    print(init_latents_w.shape)

    if args.w_mask_shape == 'circle':
        np_mask = circle_mask(32, r=5)
...
def get_p_value(reversed_latents_no_w, reversed_latents_w, watermarking_mask, gt_patch, args):
    ...
    # no_w
    reversed_latents_no_w_fft = torch.cat([reversed_latents_no_w_fft.real, reversed_latents_no_w_fft.imag])
    ...
    # w
    reversed_latents_w_fft = torch.cat([reversed_latents_w_fft.real, reversed_latents_w_fft.imag])
