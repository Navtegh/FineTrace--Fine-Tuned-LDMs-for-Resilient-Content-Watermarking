...
class PatchAveragingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        pad_h_half = pad_h // 2
        pad_w_half = pad_w // 2
        x_padded = F.pad(x, (pad_w_half, pad_w_half, pad_h_half, pad_h_half), mode='constant', value=1)
...
class HiddenDecoder(nn.Module):
    ...
    def __init__(self, num_blocks, num_bits, channels,patch_size=32, redundancy=1):
        ...
        # layers.append(ConvBNRelu(channels, 1)) #-> 16 1 256 256 

        # layers.append(nn.AdaptiveAvgPool2d(output_size=(29, 29))) #-> 16 32 1 1
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(256 * 32 * 32, 1024),  # Adjust input size for flattened layer
            nn.ReLU(),
            nn.Linear(1024, 29 * 29),  # Output matches watermark size
    ...
    def forward(self, img_w):
        x = self.patch_avg(img_w)
        return self.decoder(x).view(-1, 1, 29, 29) 
    # def forward(self, img_w):
    #     x = self.patch_avg(img_w)
    #     # print(x.shape)
    #     # print('after padding shape upar')
    #     # x = dct.dct_2d(x)
    #     x = self.decoder(x) # 16 1 256 256 
    #     # x = self.layers(x) # 16 1 256 256 
