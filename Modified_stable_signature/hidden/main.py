...
def get_parser():
    ...
    aa("--output_dir", type=str, default="output_i0/", help="Output directory for logs and images (Default: /output)")
    ...
    aa('--saveckp_freq', default=5, type=int)
    ...
    aa("--lambda_w", type=float, default=10, help="Weight of the watermark loss. (Default: 1.0)")
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        msgs = torch.zeros((imgs.shape[0],imgs.shape[-1],imgs.shape[-1])).to(device) # b k k
       
        # msgs_ori = torch.rand((imgs.shape[0],params.num_bits,params.num_bits)) > 0.5 # b k k
        key = generate_radial_gradient_image(imgs.shape[-1]).to(device)
        # key =(2*key)-1
        # single_msg = torch.rand((params.num_bits, params.num_bits)) > 0.5  # Shape: (32, 32)
        # msgs_ori = single_msg.unsqueeze(0).expand(imgs.shape[0], -1, -1)
        # msgs = 2 * msgs_ori.type(torch.float).to(device) - 1 # b k k
      ...
      #   False, False, False, False, False]]).to(device)
        ...
        save_image(fts[0,:,:],'fts.png')
        msgs1=torch.zeros_like(fts)
        for i in range(fts.shape[0]):
          size=fts[i].shape[-1]
          msgs1[i] = generate_radial_gradient_image(size).to(device)
        # print(msgs1[:].shape)
        loss_w = message_loss(fts, msgs1, m=params.loss_margin, loss_type=params.loss_w_type) # b k -> 1
        ...
        diff=torch.abs(msgs1-fts)
        bit_accs = 1-torch.sum(torch.sum(diff, dim=-1),dim=-1) / (diff.shape[-1]**2) # b k -> b
        word_accs = (bit_accs == 1) # b
        norm = torch.norm(fts, dim=(-2, -1), keepdim=True) # b d -> b 1
        log_stats = {
            'loss_w': loss_w.item(),
            'loss_i': loss_i.item(),
            'loss': loss.item(),
...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
      ...
      #   False, False, False, False, False]]).to(device)
        ...
        save_image(fts[0,:,:],'fts.png')
        save_image(msgs[0,:,:],"msgs.png")
        loss_w = message_loss(fts, msgs1,  m=params.loss_margin, loss_type=params.loss_w_type) # b -> 1
        loss_i = image_loss(imgs_w, imgs, loss_type=params.loss_i_type) # b c h w -> 1
