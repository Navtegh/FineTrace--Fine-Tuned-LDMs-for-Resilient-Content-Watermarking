...
class HiddenAug(nn.Module):
    ...
    def __init__(self, img_size, p_crop=0.0, p_blur=0.0, p_jpeg=0.0, p_rot=0.0, p_color_jitter=0.0, p_res=0.0):
        super().__init__()
        augmentations = []
        hflip = K.RandomHorizontalFlip(p=1)
        augmentations += [nn.Identity(), hflip]
        if p_crop > 0:
            crop1 = int(img_size * np.sqrt(0.1))
            crop2 = int(img_size * np.sqrt(0.5))
        ...
        if p_rot > 0:
            # aff1 = K.RandomAffine(degrees=(-25,25), p=1)
        ...
        if p_color_jitter > 0:
            ...
            jitter5 = K.ColorJiggle(brightness=(2.0, 2.0), contrast=0, saturation=0, hue=0, p=1)
            # augmentations += [jitter1, jitter2, jitter3, jitter4, jitter5]
            augmentations += [jitter1, jitter5]
