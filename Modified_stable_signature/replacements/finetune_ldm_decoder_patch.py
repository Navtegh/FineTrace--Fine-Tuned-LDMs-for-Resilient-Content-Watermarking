...
def get_parser():
    ...
    aa("--num_bits", type=int, default=32, help="Number of bits in the watermark")
    ...
    aa("--batch_size", type=int, default=1, help="Batch size for training")
    ...
    aa("--loss_w", type=str, default="mse", help="Type of loss for the watermark loss. Can be mse or bce")
    aa("--lambda_i", type=float, default=0.025, help="Weight of the image loss in the total loss")
    ...
    aa("--steps", type=int, default=3000, help="Number of steps to train the model for")
...
def main(params):
    ...
    if 'torchscript' in params.msg_decoder_path: 
        ...
        with torch.no_grad():
            ...
            #     x = x.to(device)
            #     y = msg_decoder(x)
            #     ys.append(y.to('cpu'))# 16,32,32 for 1 batch
            # ys = torch.cat(ys, dim=0) # 16*n,32,32 for n batches
            # nbit = ys.shape[1] # 32

            # #new whitening
            # ys_flat = ys.view(ys.shape[0], -1)  # Shape: (N, d*d)
    
            # # Whitening
            # mean = ys_flat.mean(dim=0, keepdim=True)  # Shape: (1, d*d)
            # ys_centered = ys_flat - mean  # Shape: (N, d*d)
            # cov = ys_centered.T @ ys_centered  # Shape: (d*d, d*d)
            
            # # Eigen decomposition and whitening transformation
            # e, v = torch.linalg.eigh(cov)
            # L = torch.diag(1.0 / torch.pow(e, exponent=0.5))  # Shape: (d*d, d*d)
            # weight = torch.mm(L, v.T)  # Shape: (d*d, d*d)
            # bias = -torch.mm(mean, weight.T).squeeze(0)  # Shape: (d*d,)
            
            # # Define the linear layer
            # linear = nn.Linear(nbit*nbit, nbit*nbit, bias=True)
            # linear.weight.data = np.sqrt(nbit*nbit) * weight
            # linear.bias.data = np.sqrt(nbit*nbit) * bias

            # Wrap the decoder and whitening in a sequential module
            msg_decoder = nn.Sequential(
                msg_decoder
                # nn.Flatten(start_dim=1),  # Flatten (N, d, d) to (N, d*d)
                # linear.to(device),
                # nn.Unflatten(1, (nbit, nbit))  # Reshape back to (N, d, d)
            )
            ...
            # # whitening
            # mean = ys.mean(dim=0, keepdim=True) # NxD -> 1xD # 1,32,32
            # ys_centered = ys - mean # NxD # 16*n,32,32
            # cov = ys_centered.T @ ys_centered # 32*32,32*32
            # e, v = torch.linalg.eigh(cov)
    ...
    if params.loss_w == 'mse':        
        loss_w = lambda decoded, keys, temp=1: torch.mean((decoded*temp - keys)**2) # b k - b k
    ...
    for ii_key in range(params.num_keys):
        ...
        # key = torch.randint(0, 2, (1, nbit), dtype=torch.float32, device=device) # 1,32 with 0s and 1s
        # key_str = "".join([ str(int(ii)) for ii in key.tolist()[0]])
        # print(f'Key: {key_str}')
        msgs = torch.zeros((params.batch_size,32,32)).to(device) # b k k
        # nbit=torch.tensor(32)
        # msgs_ori = torch.rand((imgs.shape[0],params.num_bits,params.num_bits)) > 0.5 # b k k
        # key = generate_radial_gradient_image(256).to(device)
        # single_msg = torch.rand((nbit, nbit)) > 0.5
        # msgs = 2 * single_msg.type(torch.float).to(device) - 1
    ...
    #     False, False, False, False, False, False, False, False, False,
    #     False, False, False, False, False, False, False, False, False,
    #     False, False, False, False, False]]).to(device)
        ...
        train_stats = train(train_loader, optimizer, loss_w, loss_i, ldm_ae, ldm_decoder, msg_decoder, vqgan_to_imnet, msgs, params)
        val_stats = val(val_loader, ldm_ae, ldm_decoder, msg_decoder, vqgan_to_imnet,msgs, params)
        ...
        with (Path(params.output_dir) / "keys.txt").open("a") as f:
            f.write(os.path.join(params.output_dir, f"checkpoint_{ii_key:03d}.pth") + "\t" +  "\n")
        print('\n')
...
def train(data_loader: Iterable, optimizer: torch.optim.Optimizer, loss_w: Callable, loss_i: Callable, ldm_ae: AutoencoderKL, ldm_decoder:AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet:nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    for ii, imgs in enumerate(metric_logger.log_every(data_loader, params.log_freq, header)):
        imgs = imgs.to(device)
        # keys = key.repeat(imgs.shape[0], 1, 1)
        ...
        # nan_mask = torch.isnan(decoded)  # Shape: (16, 32, 32), True where NaN
        # nan_indices = torch.nonzero(nan_mask, as_tuple=False)  # Shape: (num_nans, 3)

        # # Output the indices
        # print(f"Indices of NaN values: {nan_indices}")
        # compute loss
        lossw = loss_w(decoded, key)
        ...
        # log stats
        diff=torch.abs(key-decoded)
        diff = (~torch.logical_xor(decoded>0, key>0)) # b k -> b k
        bit_accs = (torch.sum(diff, dim=(-2,-1)) / (diff.shape[-1]**2 ))# b k -> b
...
def val(data_loader: Iterable, ldm_ae: AutoencoderKL, ldm_decoder: AutoencoderKL, msg_decoder: nn.Module, vqgan_to_imnet:nn.Module, key: torch.Tensor, params: argparse.Namespace):
    ...
    for ii, imgs in enumerate(metric_logger.log_every(data_loader, params.log_freq, header)):
        ...
        attacks = {
            ...
            # 'combined':lambda x: utils_img.center_crop(utils_img.adjust_brightness(x, 1.5),0.5)

            'combined':lambda x: utils_img.center_crop(utils_img.jpeg_compress(utils_img.adjust_brightness(x, 1.5),80),0.5)
           # 'combined':lambda x: utils_img.jpeg_compress(utils_img.adjust_brightness(utils_img.center_crop(x, 0.5),1.5),80)
        }
        for name, attack in attacks.items():
            imgs_aug = attack(vqgan_to_imnet(imgs_w))
            decoded = msg_decoder(imgs_aug) # b c h w -> b k
            # msgs1=torch.zeros_like(decoded)
            # for i in range(decoded.shape[0]):
            #     size=decoded[i].shape[-1]
            #     msgs1[i] = generate_radial_gradient_image(size).to(device)
            # print(msgs1[:].shape)
            diff = (~torch.logical_xor(decoded>0, key>0)) # b k -> b k
            # diff=torch.abs(msgs1-decoded)
            bit_accs = (torch.sum(diff, dim=(-2,-1)) / (diff.shape[-1]**2 ))# b k -> b
            # bit_accs = torch.sum(diff, dim=(-2,-1)) / (diff.shape[-1]**2) # b k -> b
            word_accs = (bit_accs == 1) # b
