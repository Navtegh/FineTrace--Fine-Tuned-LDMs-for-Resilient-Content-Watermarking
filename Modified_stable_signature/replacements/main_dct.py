...
def get_parser():
    ...
    aa('--saveckp_freq', default=10, type=int)
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
       ...
       [False, False, False, False, False, False, False, False, False,
        ...
        # msgs=msgs*0.1
        #msgs[:]=key* 0.1
        msgs,patch=get_watermarking_pattern(imgs.shape[0],4,256,256,100,3,device)
        #msgs = 2 * msgs.type(torch.float).to(device) - 1
        patch = 2 * patch.type(torch.float).to(device) - 1
        patch=patch.repeat(imgs.shape[0],1,1)
        
        fts, (imgs_w, imgs_aug) = encoder_decoder(imgs, msgs)
        save_image(fts[0,:,:],'fts.png')
        decoded_key=dct.dct_2d(fts)
        # print(decoded_key[:,0,1])
        # print('upar decoded key')
        # print(patch[:,0,1])
        patch[:,0,0]=1
        save_image(decoded_key[0,:,:],'decoded.png')
        loss_w = message_loss(decoded_key, patch, m=params.loss_margin, loss_type=params.loss_w_type) # b k -> 1
        ...
        # msg stats
        ori_msgs = torch.sign(patch) > 0
        decoded_msgs = torch.sign(decoded_key) > 0 # b k -> b k
        diff = (~torch.logical_xor(ori_msgs, decoded_msgs)) # b k -> b k
        bit_accs = torch.sum(torch.sum(diff, dim=-1),dim=-1) / (diff.shape[-1]**2) # b k -> b
        word_accs = (bit_accs == 1) # b
        norm = torch.norm(decoded_key, dim=(-2, -1), keepdim=True) # b d -> b 1
        log_stats = {
            'loss_w': loss_w.item(),
            'loss_i': loss_i.item(),
            'loss': loss.item(),
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
       ...
       [False, False, False, False, False, False, False, False, False,
        ...
        for name, attack in attacks.items():
            fts, (_) = encoder_decoder(imgs, patch, eval_mode=True, eval_aug=attack)
            decoded_key=dct.dct_2d(fts)
            decoded_msgs = torch.sign(decoded_key) > 0 # b k -> b k
            diff = (~torch.logical_xor(ori_msgs, decoded_msgs)) # b k -> b k
