...
class HiddenEncoder(nn.Module):
    ...
    def __init__(self, num_blocks, num_bits, channels, last_tanh=True):
        ...
        self.after_concat_layer = ConvBNRelu(channels + 3 + 16, channels)
    ...
    def forward(self, imgs, msgs):
        ...
        # msgs=msgs.repeat_interleave(8, dim=1).repeat_interleave(8, dim=2) # b h w
        # msgs=msgs.repeat(1,8,8)
        # msgs = msgs.unsqueeze(1).repeat(1, 1, 1, 1) # b 8 h w
        msgs = msgs.unsqueeze(1).repeat(1, 16, 1, 1) # b 1 h w
        # msgsnot = msgs*-1 # b 8 h w
...
class PatchAveragingLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        pad_h_half = pad_h // 2
        pad_w_half = pad_w // 2
        x_padded = F.pad(x, (pad_w_half, pad_w_half, pad_h_half, pad_h_half), mode='constant', value=0)
...
class HiddenDecoder(nn.Module):
    ...
    def __init__(self, num_blocks, num_bits, channels,patch_size=32):
        ...
        layers.append(ConvBNRelu(channels, 16)) #-> 16 1 256 256 

        # layers.append(nn.AdaptiveAvgPool2d(output_size=(1, 1))) #-> 16 32 1 1
        self.layers = nn.Sequential(*layers)
        self.final_conv = nn.Conv2d(16, 1, kernel_size=1, stride=1) #16 1 256 256 
        
        self.patch_avg = PatchAveragingLayer()
    ...
    def forward(self, img_w):
        ...
        x = self.layers(img_w) # 16 16 256 256
        # print(x.shape)
        # print('upar dekhooooo')
...
class EncoderDecoder(nn.Module):
    ...
    def forward(
        ...
        # encoder
        deltas_w = self.encoder(imgs, msgs) # b c h w= b 3 h w
        ...
        if eval_mode:
            ...
            fts = self.decoder(imgs_aug) # b c h w -> b d d
